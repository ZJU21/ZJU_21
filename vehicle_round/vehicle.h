...
typedef enum
{
	START, END, PAUSE, CHANGEDIR,
	LEFTWARD,RIGHTWARD,FORWARD,BACKWARD,
	GOX, GOY,
	FINDLINEY,
	SCANQR, BROADCAST,WORK,
	GET1, GET2,
} COMMAND_TYPE;
...
class Vehicle
{
	...
	float current_x = 0, current_y=0;   // mm
...
void Vehicle::init(float x,float y,COMMAND *commands)
{
	...
	botOdometry.botPosition.position_x=x;
	botOdometry.botPosition.position_y=y;
	//start_x=x;start_y=y;
...
void Vehicle::redress_odometry()
{
	if (GraySensorsUartIoOutput.ioDigital == 127)
		botOdometry.botPosition.position_x = round((botOdometry.botPosition.position_x) / grid_size) * grid_size;
	if (GraySensorsUartIoOutput.ioDigital == 8)
		botOdometry.botPosition.position_y = round((botOdometry.botPosition.position_y) / grid_size) * grid_size;
}
...
void Vehicle::run()
{
	...
	switch (now_command->typ)
	{
		...
		case GOX:
			{
				float goal=now_command->len*grid_size;
				if (current_x<goal)
					linear_vel_x=0.2;
				else
					linear_vel_x=-0.2;
		...
		case GOY:
			{
				float goal=now_command->len*grid_size;
				if (current_y<goal)
					linear_vel_x=0,linear_vel_y=0.2;
				else
					linear_vel_x=0,linear_vel_y=-0.2;
				if (fabs(goal-current_y)<=eps)
					next_command();
				break;
			}
		...
		case SCANQR:
			{
				//Serial.println("getQRcode");
				int color=hand.scanQRcode();
				//Serial.print("color=");
		...
		case GET1:
			{
				...
				{
					if (hand.try_catchA())
						hand.catchA1(),hand.put1(catch1++);
					else if (hand.try_catchB())
						hand.catchB1(),hand.put1(catch1++);
					hand.scan1();
				}
				next_command();
				break;
		...
		case GET2:
			{
				...
				{
					if (hand.try_catchA())
						hand.catchA2(),hand.put2(catch2++);
					else if (hand.try_catchB())
						hand.catchB2(),hand.put2(catch2++);
					hand.scan2();
				}
				next_command();
				break;
...
void Vehicle::update_goal_speed()
{
	...
	if (linear_vel_x==0 && linear_vel_y==0)
		angular_vel_z=0;
